...
@section('modal')
    @include('admin.libraries.Libraries.create',['id'=>'createmodal','name'=>'اضافه مكتبه جديده','action'=>url()->current()])
@stop
@endif
        ...
        $(document).ready(function () {
            ...
            var dataListView = $(".data-list-view").DataTable({
                ...
                buttons: [
                        @if (auth()->User()->group->categories_add == 1)
                    {
                        text: "<i class='feather icon-plus'></i> اضافه جديد  ",
                        action: function () {
                ...
                "columns": [
                    ...
                    {
                        "mRender": function (data, type, row) {
                            return  '<div class="avatar  avatar-xl"><img src="{{url('storage')}}'+row.file+'" alt="Img placeholder"></div>';
                        }
                    },

                    {data: 'description_ar', name: 'description_ar'},
                    {data: 'library_main_cat_name', name: 'library_main_cat_name'},
                    {data: 'library_sub_cat_name', name: 'library_sub_cat_name'},
